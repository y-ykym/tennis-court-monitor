...
//     LABEL_A / LABEL_B(予約者の呼び名)があれば「<呼び名>で予約」ボタンを人ごとに出す。これは postback ボタンで、
//     押すとブラウザは開かず、Worker が自宅サーバーに予約開始を指示して「受け付けました」と返信する(結果はカードで届く)。
//     どちらも無ければ従来どおり URL 型の「予約」1つ(サーバー側で予約者を選ぶ画面が出る)。
...
function bookingButtons(slot, cfg) {
  ...
  // 予約者が分からない旧方式: URL を開いてサーバー側で予約者を選ぶ
  if (!cfg.people) {
    const uri = `${cfg.base}/book?token=${tokenLib.sign(base, cfg.secret)}`;
    return [{ ...button('予約', { type: 'uri', label: '予約', uri }, PERSON_COLORS.A), flex: 0, margin: 'sm' }];
  }
  // 予約者ごとのボタンは postback: ブラウザを開かず、Worker が自宅サーバーに予約開始を指示して「受け付けました」と返信する。
  ...
  return cfg.people.map((p) => {
    ...
    const action =
      data.length <= 300
        ...
        : { type: 'uri', label, uri: `${cfg.base}/book?token=${token}` }; // 万一長すぎたら従来の URL 方式
