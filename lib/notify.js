...
function bookingButtons(slot, cfg) {
  ...
  if (!cfg.people) return [{ ...button('予約', base), flex: 0, margin: 'sm' }];
  // ボタンの文字は LINE の上限(20文字)に収める
  return cfg.people.map((p) => button(`${p.label}で予約`.slice(0, 20), { ...base, person: p.person }));
}
...
//   予約ボタン付きは1件あたり約1.5KB(A/B 2つ)増えるため、buildFlexMessages() が実サイズを測って1通の件数を減らし、
//   収まらない分は続きの通(最大 MAX_MESSAGES 通。push API の上限は5通)に分ける。それでも余る分だけ「…ほかN件」
const MAX_SLOTS = 12;
