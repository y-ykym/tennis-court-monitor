...
function bookingButtons(slot, cfg) {
  ...
  // displayText は押した人のメッセージとしてトークに出る(誰がどの枠を押したか全員に分かる)。data は 300 文字まで
  return cfg.people.map((p) => {
    ...
    const action =
      data.length <= 300
        ...
        : { type: 'uri', label, uri: `${cfg.base}/book?token=${token}` }; // 万一長すぎたら Worker の /book を開く(同じ処理を画面で返す)
    return button(label, action, PERSON_COLORS[p.person]);
  });
}
