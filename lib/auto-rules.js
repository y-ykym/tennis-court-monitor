...
//   - その人の同じ日の予定(都の予約・テニスベアのイベント)と時間が重なる候補は見送る(同時にはできない)。時間が接する候補は
//     同じ公園でなければ見送る(別の場所だと移動が間に合わない)。2026-09-24 追加。判定は予約直前に、都の予約一覧と
//     テニスベアの予定を実際に見て行う(booking/src/auto-runner.js)
//
...
// "HH:MM" → その日の 0:00 からの分。読めなければ null
...
// 隣接・重複の判定(2026-09-24 追加)。候補 candidate { park|facility, date, startHour|time } と、その人の予定 plans
// [{ date, start:'HH:MM', end:'HH:MM'|'', park|facility, source? }](都の予約一覧の行、またはテニスベアのイベント)を照合し、
//   - 同じ日で時間が重なる予定があれば、場所を問わず見送り(同時に 2 か所・2 面はできない)
//   - 端が接する(間の空きが gapMinutes 分以下)予定が候補と別の公園にあれば見送り(移動が間に合わない)。同じ公園なら問題なし
// 該当があれば { plan, relation: 'overlap' | 'before'(予定が候補の前) | 'after'(予定が候補の後), reason } を返す。無ければ null。
//   - 公園が分からない予定(都営以外のコート等)は別の場所として扱う
//   - 終了時刻が無い予定は defaultMinutes の長さと仮定する
//   - 候補の長さは 2 時間(サイトの 1 枠)
function findPlaceConflict(candidate, plans, { gapMinutes = AUTO_BOOKING.ADJACENT_GAP_MINUTES, defaultMinutes = AUTO_BOOKING.DEFAULT_EVENT_MINUTES } = {}) {
  const cPark = parkOf(candidate.park ?? candidate.facility);
  const cStart = minutesOf(startHHMM(candidate.startHour ?? candidate.time));
  if (!cPark || cStart == null) return null;
  const cEnd = cStart + 120;
  for (const plan of plans || []) {
    if (!plan || plan.date !== candidate.date) continue;
    const pStart = minutesOf(plan.start);
    if (pStart == null) continue;
    const pEnd = minutesOf(plan.end) ?? pStart + defaultMinutes;
    let relation = null;
    if (pStart < cEnd && cStart < pEnd) relation = 'overlap';
    else if (pEnd <= cStart && cStart - pEnd <= gapMinutes) relation = 'before';
    else if (cEnd <= pStart && pStart - cEnd <= gapMinutes) relation = 'after';
    if (!relation) continue;
    const pPark = parkOf(plan.park ?? plan.facility);
    const samePark = !!(pPark && pPark.code === cPark.code);
    if (relation !== 'overlap' && samePark) continue;
    const where = plan.facility || (pPark ? pPark.name : '別の場所');
    const what = plan.source === 'tennisbear' ? 'テニスベアの予定' : '予約';
    const when = `${plan.start}${plan.end ? `-${plan.end}` : ''}`;
    if (relation === 'overlap') return { plan, relation, reason: `時間が重なる${what}がある(${when} ${where})。同時にはできないため` };
    return { plan, relation, reason: `${relation === 'before' ? '直前' : '直後'}に別の場所の${what}がある(${when} ${where})。移動が間に合わないため` };
  }
  return null;
}
