...
//   - 除外日(A/B 共通、1 日単位)の枠は自動予約しない → 従来どおり通知する
...
// 利用日 = 今日 + PENALTY_DAYS + 1(自動予約の対象のうち、無料キャンセルが今日 23:59 までの枠)か
...
//   ペナルティ期間・締切後・除外日の枠(Pi が予約しないもの)は通知する。
//   除外枠(excluded_slot)は Pi の生死に関係なく通知しない(本人が今手放した枠の空き通知は雑音)。ただし autoState が取れなければ除外枠も分からないので全部通知
...
function splitForNotification(newSlots, { now = Date.now(), today = jstTodayIso(now), autoState } = {}) {
  ...
  for (const slot of newSlots) {
    const c = classifySlot(slot, { now, today, exclusions: ex });
    if (c.kind === 'excluded_slot') suppressed.push({ slot, ...c });
