...
const AUTO_BOOKING = {
  ...
  // POLL_INTERVAL_MS は POLL_SCHEDULE のどの時間帯にも当たらない時間(= 7:00〜翌 1:00)の間隔
  POLL_INTERVAL_MS: 60 * 1000,
  MIN_POLL_INTERVAL_MS: 30 * 1000,
  // 時間帯(JST)ごとの間隔。相手のサーバーへの負荷を減らすため、空きが出にくい時間は広げる
  //   1:00〜7:00 … 3 分(ほぼ出ない)   それ以外(7:00〜翌 1:00)… POLL_INTERVAL_MS(1 分)
  // 2026-09-17 は 7:00〜21:00 を 2 分にしていたが、state.json の履歴(8/31〜9/24)で日中も争奪枠が出ていたため
  // 2026-09-24 に 1 分へ(本人決定、お試し)。日中の成功が増えなければ { FROM: '07:00', TO: '21:00', POLL_INTERVAL_MS: 2 * 60 * 1000 } を戻す
  POLL_SCHEDULE: [
