...
運用中の現状・設定値の所在・残 TODO は `docs/引き継ぎ_フェーズ2運用.md` が正。要件の履歴は `docs/予約空き監視_要件定義書.md`(§11 1.5 / §12 1.6 / §13 3 / §14 4 / §15 5 / §16 6 / §17 7)。
...
worker/                      フェーズ1.5 予約確認ボット + 1.6 予約キャンセル + フェーズ3 の状態置き場 + フェーズ4 予告アラート + フェーズ5 テニスベア合流(Cloudflare Workers。lib/ とは独立)
  ...
  src/courts.js              都の公園名・テニスベアのコード・緯度経度の台帳(フェーズ5/6 で共用)
