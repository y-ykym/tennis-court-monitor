...
| 3 | Pi が自分で空きを照会(7 時〜翌 1 時は 1 分おき、深夜 1〜7 時は 3 分)し、対象の枠を自動予約。LINE「じどう」で除外日・除外枠 | Pi + Worker + Actions(通知の絞り込み) | 2026-09-17 |
...
| Worker(`worker/src/auto.js`) | KV に 除外日・除外枠 を持つ。Pi の生存はトンネル越しに Pi の `/auto/status` を直接聞いて判定。「じどう」で一覧カード。キャンセル成功時にその枠を除外枠に記録。Pi の求めに応じてその人のテニスベアの予定を返す(`/auto/tennisbear`。隣接の判定用) |
...
- **止める・戻す(LINE から)**: `じどうおふ` で自動予約を一時停止、`じどうおん` で再開(2026-09-17 追加)。Worker の KV にスイッチを持ち、Pi は次の照会(1〜3 分)から従う。
  停止中も照会は続き(ログに `[LINE で停止中] 予約するはず`)、空きは従来どおり通知カードで届く。「じどう」カードの状態欄に「LINE の「じどうおふ」で OFF」と出る。
  Pi 側の `.env` が `dry-run`/`off` のときは LINE から ON にはできない(カードにその旨が出る)
- **空き通知カードを止める・戻す(LINE から)**: `つうちおふ` で空き通知カードを止め、`つうちおん` で戻す(2026-09-18 追加)。Actions は Worker の `/auto/state` の `notifyEnabled` を見て全部の空き通知を止め、
  Pi も予約直前に対象外になった枠のカードを送らない。自動予約そのものと、その結果カード・フェーズ4 の予告・「よやく」の返信は止めない。「じどう」カードに「空き通知カード: 届く / 止めている」の行が出る。
