...
| 3 | Pi が自分で空きを照会(7 時〜翌 1 時は 1 分おき、深夜 1〜7 時は 3 分)し、対象の枠を自動予約。LINE「せってい」で設定メニュー(自動予約・空き通知の ON/OFF、除外日・除外枠。2026-09-25 まで「じどう」) | Pi + Worker + Actions(通知の絞り込み) | 2026-09-17 |
...
| 6 | 「よやく」の予約一覧に、その枠の時間帯の天気(気温・降水確率)を出す | 同 Worker | 2026-09-18 |
| 7 | LINE で「うけつけ」→ 受付で見せる利用者カードの画像(スマホで撮ったスクショ)を人数分だけ返す | 同 Worker | 2026-09-18 |
| 8 | LINE で「きゃんせる」→ 監視対象の都営コート 3 公園の公園名と電話番号のカード(行をタップで電話) | 同 Worker | 2026-09-24 |
| 9 | LINE で「へるぷ」→ この bot が反応する合言葉の一覧カード(行をタップでその合言葉を送る) | 同 Worker | 2026-09-25 |

運用中の現状・設定値の所在・残 TODO は `docs/引き継ぎ_フェーズ2運用.md` が正。要件の履歴は `docs/予約空き監視_要件定義書.md`(§11 1.5 / §12 1.6 / §13 3 / §14 4 / §15 5 / §16 6 / §17 7 / §18 8 / §19 9)。
...
worker/                      フェーズ1.5 予約確認ボット + 1.6 予約キャンセル + フェーズ3 の状態置き場 + フェーズ4 予告アラート + フェーズ5 テニスベア合流(Cloudflare Workers。lib/ とは独立)
  ...
  src/weather.js             フェーズ6: Open-Meteo から枠の時間帯の天気を取る(Cache API に 30 分。失敗しても一覧は出す)
  src/courts.js              都の公園名・テニスベアのコード・緯度経度・サービスセンターの電話番号の台帳(フェーズ5/6/8 で共用)
  src/card.js                フェーズ7: 「うけつけ」への返信(カード画像を人数分)と、画像の口 GET /card/<A|B>.png(署名付き)。
                             KV に入れたカード画像を返し、無ければその場でバーコードだけ作る。予約サイトには行かない
  src/contacts.js            フェーズ8: 「きゃんせる」への返信(監視対象 3 公園の公園名と電話番号のカード。行をタップで電話)。
                             番号は courts.js の台帳(phone)。予約サイト・KV には行かない
  src/help.js                フェーズ9: 「へるぷ」への返信(合言葉の一覧カード。行をタップでその合言葉を送る)。
                             合言葉は各モジュールの定数を import して出す。予約サイト・KV には行かない
...
- 返信の見た目: **人ごとに 1 枚のカードを横に並べたカルーセル**(スワイプで切り替え。1 人なら 1 枚)。各カードは濃紺ヘッダーに名前と「M/D 現在 ・ N件」、
  日付タイル(土=青/日祝=赤/平日=グレー)+時間+公園名。**同じ日の予定は 1 つのタイルの右に時間順で縦に並ぶ**(2026-09-17 変更。設計の見本は `docs/flex-design-grouped.html`)。
  直近は「今日/明日」、当日で終了した枠はグレー+「終了」(同じ日に終了と未了が混ざればタイルは通常色、終了した枝だけグレー)。
...
- 同じ日に都の予約とテニスベアの予定があれば、1 つの日付タイルの右に時間順で縦に並ぶ(2026-09-17 変更。その日に何が入っているかが一目で分かる)
...
監視中の枠に空きが出たら、自宅の Raspberry Pi が **定期的に自分で空きを照会し(7 時〜翌 1 時は 1 分おき、深夜 1〜7 時は 3 分)、見つけたら同じプロセスで即予約**します。
...
| 場所 | 役割 |
|---|---|
| Pi(`booking/src/auto-runner.js`) | 定期的に(7 時〜翌 1 時は 1 分、深夜 1〜7 時は 3 分): 空き照会(`lib/scrape.js`、ログイン不要)→ 監視条件 → 前回との差分 → Worker に heartbeat(応答で除外日・除外枠を受け取る。KV には書かない)→ `lib/auto-rules.js` で振り分け → 予約の行列へ。行列は 1 件ずつ実行し、LINE の予約ボタン(手動)を優先 |
...
| Worker(`worker/src/auto.js`) | KV に 除外日・除外枠 を持つ。Pi の生存はトンネル越しに Pi の `/auto/status` を直接聞いて判定。「せってい」で設定メニューのカード。キャンセル成功時にその枠を除外枠に記録。Pi の求めに応じてその人のテニスベアの予定を返す(`/auto/tennisbear`。隣接の判定用) |
...
- **上限**: 利用日ごとに 1 件(手動で取った分も数える。**テニスベアのイベントは数えない**。2026-09-17 に 2 → 1)。予約直前にログインして予約一覧を見て数える。同じ実行で複数候補があれば成立分も数え、達したら残りはログインせず見送る
- **重なり・隣接(2026-09-24 追加)**: その予約者の同じ日の予定(都の予約・テニスベアのイベント)と**時間が重なる**候補は、場所を問わず自動予約しない(同時にはできない)。
  **時間が接する(前の予定の終了 = 候補の開始、または候補の終了 = 次の予定の開始)**候補は、**同じ公園でなければ自動予約しない**(別の場所だと移動が間に合わないため)。
  同じ公園に接しているだけなら予約する。A・B それぞれ自分の予定だけを見る。
  テニスベアの予定は予約直前に Worker 経由で取り(`/auto/tennisbear`。10 分は使い回す)、隣に別の場所の予定があれば**ログインせず**見送る。
  都の予約との隣接は、ログイン直後の予約一覧で見る(上限が 1 件のうちは先に上限で見送られるので、実際に効くのはテニスベアの方)。
  テニスベアの予定が取れないときは直近 60 分以内の結果で判定し、それも無ければ見送る(判定できないため予約しない)。
  都営以外のコートのイベントは「別の場所」。終了時刻が取れないイベントは 2 時間と仮定。接する判定の許容(間の空き)は `ADJACENT_GAP_MINUTES`(既定 0 分)。見送りはカードなし・ログのみ
...
- **通知**: 成功は結果カードの見出しを「🤖 自動予約が完了」にして送る(手動は「🎾 予約完了(ボタン)」。2026-09-25 に見分けやすく変更)。失敗のうち「先に予約されていました」「サイトが断った」「サイトのエラー」は **カードを送らずログのみ**
...
- **止める・戻す(LINE から)**: 「せってい」カードの「自動予約」行の **「OFF にする」「ON にする」ボタン**(2026-09-25 追加。押すと `じどうおふ` / `じどうおん` が送られる。打ち込んでも同じ。2026-09-17 追加)。Worker の KV にスイッチを持ち、Pi は次の照会(1〜3 分)から従う。
  停止中も照会は続き(ログに `[LINE で停止中] 予約するはず`)、空きは従来どおり通知カードで届く。「せってい」カードの「自動予約」行が「OFF 停止中」になる。
  Pi 側の `.env` が `dry-run`/`off` のときは LINE から ON にはできない(カードにその旨が出る)
- **空き通知カードを止める・戻す(LINE から)**: 「せってい」カードの「空き通知カード」行の **「OFF にする」「ON にする」ボタン**(押すと `つうちおふ` / `つうちおん` が送られる。打ち込んでも同じ。2026-09-18 追加)。Actions は Worker の `/auto/state` の `notifyEnabled` を見て全部の空き通知を止め、
  Pi も予約直前に対象外になった枠のカードを送らない。自動予約そのものと、その結果カード・フェーズ4 の予告・「よやく」の返信は止めない。「せってい」カードの「空き通知カード」行に ON / OFF が出る。
  OFF の間に出た空きは state.json に記録されるので、ON に戻しても遡って通知はされない
- **設定メニュー**: LINE で `せってい`(2026-09-25 まで `じどう`)→ ⚙️ 設定カード。上に「自動予約」「空き通知カード」のスイッチ 2 行(ON/OFF と Pi の状態、右に切り替えボタン)、下に除外日・除外枠の一覧。フッターの「↻ 更新」で最新にする
- **除外日**: 「せってい」カードのフッター「📅 除外日を追加」(日付ピッカー)で追加、各行の「解除」で戻す。除外日の枠は自動予約せず、**空き通知もしない**(2026-09-17 に「通知もしない」へ変更)。過ぎた日は自動で消える
...
Worker は `cd worker && npm test && npx wrangler deploy`(「せってい」と `/auto/*` が入る。Secrets の追加なし)。Actions は push で次回から反映(Secrets の追加なし。
...
- **止めたいとき**: LINE で `じどうおふ`(1〜3 分で反映。戻すのは `じどうおん`)。Pi を触るなら `.env` を `AUTO_BOOKING=dry-run`(照会だけ)または `off` にして `docker compose up -d`(再ビルド不要)。どちらも Actions は自動で「Pi は停止中」と判断し全部通知に戻る
- **狙った枠で試したいとき**(実枠テストの手順。本物の予約になるので了解を得てから):
  ...
  3. 1〜2 分で `[auto] 自動予約 開始` → `結果: …` がログに出る。成功なら「🤖 自動予約が完了」カード。要らなければ「よやく」→「キャンセル」で取り消す(除外枠に載る)
...
- `テニスベアの予定を取得: 予約者 A は今後 N 件(隣接の判定に使う)` → `見送り: …(直前に別の場所のテニスベアの予定がある(11:00-13:00 亀戸中央公園)。移動が間に合わないため。ログインせず)` … 隣接で見送り(2026-09-24 追加)。
  `テニスベアの予定が取れず、…判定できないため` は Worker かテニスベアの不調(直近 60 分の結果も無いとき)
...
- 空き照会(ログイン不要)は 1 回 18 リクエスト(3 公園 × (トップ + 検索 + 4 週))。Pi は 3 公園を並行して取る(`AUTO_SCRAPE_CONCURRENCY`、既定 3。夜はサイトが遅く順番だと 100 秒かかった)。
  ...
  **間隔は時間帯(JST)で変える**(`AUTO_BOOKING.POLL_SCHEDULE`): 7:00〜翌 1:00 は 1 分、1:00〜7:00 は 3 分。
  2026-09-17 は 7:00〜21:00 を 2 分にしていたが、state.json の履歴(8/31〜9/24)を見ると日中も出現の半分が 3 分以内に消える争奪枠だったため、
  2026-09-24 に 1 分へ(本人決定、お試し。見積もりでは日中の取得が 24 日で 4 件ほど増える程度)。日中の成功が増えなければ 2 分に戻す
  失敗時のレスポンスはコンテナに溜めない(`SCRAPE_DEBUG_DIR=''`)
- サーバーへの負荷の目安(2026-09-24 時点): Actions 3 分おき ≒ 1 日 8,600 回、Pi ≒ 1 日 21,600 回。合計で平均 1 秒に 0.35 回、画像やスクリプトは取らない軽い照会のみ。
  ピーク(21〜翌 1 時)は 2026-09-17 の設定から変わっていない
- ログインを伴うのは予約の実行時だけ(候補 1 件につき 1 回)。dry-run では一切ログインしない
- **ログインが拒否された(auth_error)予約者は 6 時間その人の自動予約を止める**(パスワード誤り等で繰り返すとアカウントロックの恐れ。カードは最初の 1 回だけ。
  6 時間後に 1 回だけ試し直す)。カードが届いたら「よやく」で同じ人の一覧が取れるか確かめる(取れなければ利用者番号・パスワード・カードの有効期限、取れれば Pi 側の一時的な問題)
...
- **LINE の push は月 200 通(グループ宛は 1 回 2 通)**。フェーズ3 は「成功だけカード、負け・見送り・サイトエラーはログのみ」、フェーズ4 は「0 件なら送らない、23:35 は朝と同じなら送らない」で節約している。
  10 月の見込みは 100〜150 通。行けない日は「せってい」で先に除外日にしておくと、成立しない分だけ成功カードとフェーズ4 の予告も減る
...
- 一覧の取得はフェーズ1.5 と同じ経路(A・B 並行)ですが、Cron は LINE の返信期限に縛られないため 90 秒まで待ち、開始 60 秒以内の失敗は再試行します(2026-09-25 に予約サイトの応答が遅く 25 秒で打ち切られた反省)。Cron からの実行なので LINE の replyToken は使わず push します。
- 施設によって `penaltyday` が 3 以外の場合も、その値で境目を計算します(一覧から取れなかったときだけ 3 として扱います)。

---
## フェーズ7 利用者カード(2026-09-18 稼働開始)

施設の受付で使うカードを LINE で出します。**受付のスキャナにバーコードを読ませる**用と、
**代理受付で利用者番号を読み上げる**用です。要件と調査の記録は要件定義書 §17。

### 使い方

グループで **`うけつけ`** と送ると、**カードの画像が人数分だけ**返ります(文章は添えません)。
タップすると全画面になるので、そのまま受付のスキャナに見せられます。

画像は**本人がスマホで撮った「利用者カード」画面のスクリーンショットそのもの**です
(画面全体。ブラウザの URL 欄や時計も写っています)。Flex Message では余白・文字の太さ・縞の比率まで
再現しきれなかったため、画像を配る形にしました(2026-09-18 本人判断)。

### 仕組み(要点)

- **予約サイトにはログインしません。** バーコードの中身は利用者番号 8 桁そのままで、それは既に Secrets
  (`SITE_USER_A` / `SITE_USER_B`)にあります。だから返信は 1 秒以内に返り、現地の電波に左右されません
- カードの中身(番号・氏名・縞)は変わらないので、**画像は 1 度作って KV に入れて使い回します**。
  作り直すのは氏名や番号が変わったときだけです
- 画像は `GET /card/<A|B>.png?s=<署名>` で返します。KV に画像が無い人には**画像を送らず、利用者番号だけ**を
  テキストで知らせます(受付で読み上げはできます。壊れた画像を出さないため)
- 画像 URL の署名に**期限はありません**。トーク履歴に残ったカードを後日そのまま見せられるようにするためです
  (受付は電波が悪いことがあるため)

### カード画像の用意のしかた

**本名が入るのでリポジトリには置きません。** スクショを撮って KV に入れます。

1. スマホで予約サイトにログインし、ヘッダーの「利用者カード表示」を押して**画面をスクショ**します
2. その画像を Mac に送ります(AirDrop など)
3. **1MB を超えていたら縮めます**(LINE のプレビュー画像は 1MB まで)。macOS 標準の `sips` でできます

   ```bash
   sips --resampleWidth 1000 元の.png --out card-A.png
   ```

4. KV に入れます

   ```bash
   cd worker && npx wrangler kv key put --binding BOOKING_KV --remote card_image_A --path card-A.png
   ```

5. B の分も同じように(`card_image_B`)

縮めたあとは**バーコードがまだ読めるか**を確かめてください(スマホのバーコードリーダーで、
読み取った値が利用者番号と一致すれば OK)。

### 有効化の手順(初回)

1. 上の「カード画像の用意のしかた」で A・B の画像を KV に入れます
...
3. グループで `うけつけ` と送り、カードの画像が 2 枚返ることを確認します

4. **バーコードが受付のスキャナで読めるか**を確かめます。手元では次で確認できます
   - スマホのカメラアプリや任意のバーコードリーダーで、LINE に出たバーコードを読む
   - 読み取った値が**利用者番号と一致**すれば OK(画面の明るさは上げておく)

### 気をつけること

- `BOOKING_SIGNING_SECRET` が無いと画像に署名できないので、**番号だけのテキスト**で返します(読み上げはできます)
- 利用者番号が **CODE128 にできない形式**(奇数桁・数字以外)の人は、そもそも対象から外します
- 氏名や利用者番号が変わったら、**画像を作り直して KV に入れ直します**(古い画像が出続けます)
- 利用者カードの有効期限が切れると、サイト側ではログインできなくなりますが、**この画像は出続けます**。
  期限切れは「よやく」が「取得失敗」になることで気づけます(README のフェーズ1.5 のトラブル時を参照)

## フェーズ8 コートの連絡先(2026-09-24 稼働開始)

電話でキャンセルの連絡をするときに、公園の電話番号をすぐ引けるようにします。要件は要件定義書 §18。

### 使い方

グループで **`きゃんせる`** と送ると、監視対象の都営コート 3 公園(猿江恩賜公園・亀戸中央公園・大島小松川公園)の
**公園名とサービスセンターの電話番号**が 1 枚のカードで返ります。**行(番号)をタップすると電話アプリが開きます**
(LINE の `tel:` リンク)。フッターのボタンはありません。

### 仕組み(要点)

- 番号は `worker/src/courts.js` の台帳(`phone`)に持っています。**予約サイト・KV・Secrets には触らない**ので返信は 1 秒以内です
- 番号の出典は東京都公園協会「公園へ行こう!」の連絡先一覧(https://www.tokyo-park.or.jp/inquiry/list_park/ 。2026-09-24 確認)
- Flex が 400 で弾かれたときは、同じ内容をテキスト(1 公園 1 行)で再送します
- 公園を増やしたら台帳に `phone` も足します。`phone` が無い公園はカードに出ません

### 有効化の手順(2026-09-24 に実施済み。コードを変えたときの再確認手順)
...
2. グループで `きゃんせる` と送り、3 公園の名前と番号が返ること、番号をタップすると電話アプリが開くことを確認します

### 気をつけること

- 電話番号が変わったら `worker/src/courts.js` の `phone` を直して deploy し直します(カードは台帳の値をそのまま出します)
- 受付時間はカードに出していません(公式ページで確認できたのは番号だけのため)。必要なら台帳に足して出せます

## フェーズ9 使い方カード(2026-09-25 稼働開始)

合言葉が増えて覚えきれなくなったので、「どの言葉を送ると何が返るか」を LINE で引けるようにします。要件は要件定義書 §19。

### 使い方

グループで **`へるぷ`** と送ると、この bot が反応する合言葉の一覧が 1 枚のカードで返ります。

| 合言葉 | 見出し | 説明 |
|---|---|---|
| `よやく` | 予約の一覧 | 都営コートとテニスベアの予定を日付順に。天気も出ます。キャンセルもここから |
| `せってい` | 設定 | 自動予約と空き通知を止める・戻す。予約したくない日を決める |
| `うけつけ` | 受付で見せるカード | 利用者カードの画像を人数分 |
| `きゃんせる` | 電話でキャンセル | 都営コートの電話番号。タップでかかります |
| `へるぷ` | この一覧 | 合言葉と、送ると何が返るか |

文言は読む人(家族・仲間)の目線で「したいこと」を見出しにし、仕組みの用語(除外枠・ON/OFF など)は使いません。
ON/OFF の打ち込み合言葉(`じどうおん` など)は設定カードのボタンで押せるので載せていません(2026-09-25 実機確認後に本人決定)。
**行をタップすると、その合言葉がグループに送られ、そのまま bot が反応します**(LINE の message アクション)。フッターのボタンはありません。

### 仕組み(要点)

- 合言葉の文字列は各モジュールの定数(`COMMAND_TEXT`、`AUTO_COMMAND_TEXT`、`CARD_COMMAND_TEXT`、`CONTACT_COMMAND_TEXT`)を `worker/src/help.js` が import して出します。
  合言葉を変えたらカードも自動で追従します(2026-09-25 の「じどう」→「せってい」のようなときに書き写し忘れが起きないように)
- **予約サイト・KV・Secrets には触らない**ので返信は 1 秒以内です
- Flex が 400 で弾かれたときは、同じ内容をテキスト(1 合言葉 1 行)で再送します
- 合言葉を増やしたら `help.js` の `helpRows()` に 1 行足します。テスト(`worker/test/help.test.mjs`)が一覧と定数のずれを見ています

### 有効化の手順(2026-09-25 に実施済み。コードを変えたときの再確認手順)
...
2. グループで `へるぷ` と送り、5 つの合言葉が返ること、行をタップするとその合言葉が送られて bot が反応することを確認します
