...
| 7 | LINE で「うけつけ」→ 受付で見せる利用者カード(本名・利用者番号・バーコード)を人ごとのカルーセルで | 同 Worker | 2026-09-18 |
...
worker/                      フェーズ1.5 予約確認ボット + 1.6 予約キャンセル + フェーズ3 の状態置き場 + フェーズ4 予告アラート + フェーズ5 テニスベア合流(Cloudflare Workers。lib/ とは独立)
  ...
  src/card.js                フェーズ7: 「うけつけ」への返信(人ごとのカルーセル)と、バーコード画像の口 GET /card/<A|B>.png(署名付き)。
                             予約サイトには行かない(利用者番号は Secrets にあるため)
...
グループで **`うけつけ`** と送ると、人ごとのカードがカルーセル(横スワイプ)で返ります。

**見た目は予約サイトの「利用者カード」モーダルに寄せています**(受付で出すものが同じに見えるように)。
...
│                        │
│  利用者番号：00000000   │
│  利用者氏名：山田太郎 様 │  ← どちらのカードかは氏名で分かる
│                        │
│  ███▌▐█ ▌▌███▐▌ ██▌    │  ← タップで全画面(ピンチで拡大)
│        00000000        │  ← モーダルと同じく縞の下にも番号
└────────────────────────┘
```

呼び名(ゆうたそ / まきたそ)は**カードに出しません**(モーダルに無いため)。プッシュ通知の要約には残しています。

寸法と色は、実物のモーダルを Chrome で開いて **`getComputedStyle` で測った値**に合わせています(目分量ではありません)。

| 箇所 | 実物 | カード |
|---|---|---|
| カード幅 | 340px | `giga` |
| 見出し | 18px / 700 / `#232A2F` | `lg`(19px) / bold / 同色 |
| ラベル(「利用者番号：」) | 18px / **700** / `#666B6E` | `lg` / bold / 同色 |
| 値(番号・氏名) | 18px / 400 / `#666B6E` | `lg` / 標準 / 同色 |
| ラベル行の字下げ | 20+10px | 16+10px |
| 縞の比率 | 198×150 = 1.32:1 | 1188×900 = 1.32:1 |
| 縞の下の番号 | 10px / 黒 / 5px 下 | `xxs`(11px) / 黒 / 4px 下 |

LINE の Flex は px を自由に指定できないので、最も近い値を当てています。

**モーダルと違うのは「閉じる」ボタンを置かないことだけ**です(LINE のメッセージは閉じられないため)。

**ボタンは1つも置いていません**(2026-09-18 本人判断)。「よやく」のカードも変えていないので、カードを出すのは `うけつけ` と送る方法だけです。

見本: `docs/flex-design-card.png`

### 仕組み(要点)

- **予約サイトにはログインしません。** バーコードの中身は利用者番号 8 桁そのままで、それは既に Secrets
  (`SITE_USER_A` / `SITE_USER_B`)にあります。だから返信は 1 秒以内に返り、現地の電波に左右されません
- バーコードは Worker が自分で PNG にして返します(`GET /card/<A|B>.png?s=<署名>`)。
  **約 670 バイト**しかないので回線が細くてもすぐ出ます
- 画像 URL の署名に**期限はありません**。トーク履歴に残ったカードを後日そのまま見せられるようにするためです
  (受付は電波が悪いことがあるため)
- パターン表はサイトが使っている `jquery-barcode` から取った値をそのまま持っているので、**縞はサイトの表示と同じ**です

### 有効化の手順

1. 本名を登録します(受付で氏名を照合されるため。未登録なら氏名の行が出ないだけで、バーコードは出ます)
...
3. グループで `うけつけ` と送り、カードが 2 枚返ることを確認します

4. **実機のバーコードが受付のスキャナで読めるか**を確かめます。手元では次で確認できます
...
- 利用者カードの有効期限が切れると、サイト側ではログインできなくなりますが、**このカードは番号から作っているだけなので出続けます**。
