...
worker/                      フェーズ1.5 予約確認ボット + 1.6 予約キャンセル + フェーズ3 の状態置き場 + フェーズ4 予告アラート + フェーズ5 テニスベア合流(Cloudflare Workers。lib/ とは独立)
  ...
  src/barcode.js             フェーズ7: 利用者番号 → CODE128 のバーコードを PNG に(サイトが使う jquery-barcode と同じ縞。約 670 バイト)
...
| `CARD_NAME_A` / `CARD_NAME_B` | 利用者カードに出す本名(フェーズ7。サイトの「利用者氏名」と同じ。「様」はボットが付ける。未登録なら氏名の行を出さない) |
...
- 画像は `GET /card/<A|B>.png?s=<署名>` で返します。KV に画像が無い人には、Worker がその場で
  **バーコードだけの PNG**(約 670 バイト)を作って返すので、用意するまでの間も使えます
- 画像 URL の署名に**期限はありません**。トーク履歴に残ったカードを後日そのまま見せられるようにするためです
  (受付は電波が悪いことがあるため)
- 縞のパターン表はサイトが使っている `jquery-barcode` から取った値をそのまま持っているので、**サイトの表示と同じ**です
...
1. 本名を登録します(画像を作るときに使うほか、画像が無いときのテキスト返信にも出ます)

   ```bash
   cd worker && npx wrangler secret put CARD_NAME_A   # 例: 山田太郎(「様」は付けない)
   npx wrangler secret put CARD_NAME_B
   ```

2. 上の「カード画像の作り方」で A・B の画像を KV に入れます
...
4. グループで `うけつけ` と送り、カードの画像が 2 枚返ることを確認します

5. **バーコードが受付のスキャナで読めるか**を確かめます。手元では次で確認できます
   - スマホのカメラアプリや任意のバーコードリーダーで、LINE に出たバーコードを読む
   - 読み取った値が**利用者番号と一致**すれば OK(画面の明るさは上げておく)

### 気をつけること

- 利用者番号が**奇数桁・数字以外**だとバーコードにできないため、その人のカードは黙って出しません(ログに理由が出ます)。
  CODE128-C は偶数桁の数字しか入れられない仕様です
