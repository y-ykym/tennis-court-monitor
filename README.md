...
- 空き照会(ログイン不要)は 1 分に 20 リクエスト程度。Pi は 3 公園を並行して取る(`AUTO_SCRAPE_CONCURRENCY`、既定 3。夜はサイトが遅く順番だと 100 秒かかった)。
  次の照会は「前回の開始から 60 秒後」。照会が 60 秒を超えたら 15 秒だけ空けて次へ。失敗時のレスポンスはコンテナに溜めない(`SCRAPE_DEBUG_DIR=''`)
