...
- 空き照会(ログイン不要)は 1 回 18 リクエスト(3 公園 × (トップ + 検索 + 4 週))。Pi は 3 公園を並行して取る(`AUTO_SCRAPE_CONCURRENCY`、既定 3。夜はサイトが遅く順番だと 100 秒かかった)。
  次の照会は「前回の開始から 60 秒後」。照会が 60 秒を超えたら 15 秒だけ空けて次へ。**深夜 1:00〜7:00(JST)は 3 分おき**(`AUTO_BOOKING.NIGHT`。空きの出現は 23 時台に集中し深夜は少ないため)。
  失敗時のレスポンスはコンテナに溜めない(`SCRAPE_DEBUG_DIR=''`)
- サーバーへの負荷の目安(2026-09-15 時点): Actions 3 分おき ≒ 1 日 8,600 回、Pi ≒ 1 日 22,000 回。合計で平均 1 秒に 0.35 回、画像やスクリプトは取らない軽い照会のみ
