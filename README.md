...
| 合言葉 | 返るもの |
|---|---|
| `よやく` | 予約一覧(都の予約とテニスベアの予定。天気・キャンセルボタン付き) |
| `せってい` | 設定メニュー(自動予約・空き通知カードの ON/OFF、除外日・除外枠) |
| `うけつけ` | 利用者カードの画像(人数分) |
| `きゃんせる` | 都営コートの公園のサービスセンターの電話番号 |
| `へるぷ` | この一覧 |

「せってい」の行には、打ち込みでも切り替えられる合言葉(`じどうおん` / `じどうおふ` / `つうちおん` / `つうちおふ`)を小さく添えています。
