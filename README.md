...
- **通知**: 成功は既存の結果カードに「(自動予約)」を付けて送る。失敗のうち「先に予約されていました」「サイトが断った」は **カードを送らずログのみ**
  (人が何もできない失敗。LINE の月 200 通を節約。2026-09-14 決定)。ログインできない・reCAPTCHA で拒否・サイトのエラーは放置すると自動予約が止まるのでカードで知らせる。
  見送り(2 件あった・上位を取った)はカードなし。
...
- **Pi 不達の判定**(Actions 側): Pi の heartbeat が 5 分以内で、かつ mode が on のときだけ「生きている」。取れなければ全部通知(安全側)
