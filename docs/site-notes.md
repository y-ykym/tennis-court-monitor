...
## 重要: データセンター IP では reCAPTCHA v2(チェックボックス)が発動する(2026-09-03)

- ローカル(自宅回線)からのヘッドレス Playwright: 予約内容確認画面(prwea1000)は reCAPTCHA v3(バッジのみ、
  チェックボックスは display:none)で、「予約」まで自動で通り予約成立(STEP 2・STEP 5)
- **GitHub Actions(データセンター IP)からの同一コード: 予約内容確認画面に「チェックを入れてから『予約』ボタンを
  クリックしてください」が現れ、目に見える reCAPTCHA v2 チェックボックスが要求された**(予約は不成立)。
  reCAPTCHA v3 のスコアが IP 由来で下がり、v2 チャレンジにフォールバックしたとみられる
- 影響: **GitHub Actions / Cloud Run など DC IP からの完全自動予約は成立しない**。住宅 IP からの実行が必要。
  対策候補: (a) 住宅回線の self-hosted runner / ユーザーの Mac で実行、(b) 半自動(枠選択まで自動、v2 チェックだけ人間)
- 処理経路(ログイン→検索→枠選択→確認画面→結果の LINE 通知)は Actions 上でも正しく動作した。詰まりは reCAPTCHA のみ

## 検証やり直し: DC IP からの reCAPTCHA は v3 通過が大半(2026-09-03、dry-run 判定コードで計測)

先の「6/6 で v2」は誤り(集計ミス)。1件ずつ確実に計測し直した実データ:

| 実行 | reCAPTCHA |
|---|---|
| 本番(dry_run=false, STEP8) | v2 チャレンジ(要チェック)→ 予約失敗 |
| dry-run(判定コード, 1件) | v2 チャレンジ |
| dry-run 1件ずつ ×3(1回目のまとまり) | v3 のみ ×3 |
| dry-run 1件ずつ ×6(確認) | v3 のみ ×6 |

- 合計 **11回中 v3が9回・v2が2回**(v2 は約18%)。しかも v2 の2回は早い時間帯に集中し、以降の9回は連続で v3。
  GitHub Actions の共有 IP のスコアや初回コンテナ状態に左右される可能性がある
- 結論の訂正: **DC IP でも大半は v3 で完全自動が成立する**。ただし時々 v2 が出て、その回は自動確定できない
- 設計の含意: GitHub Actions での完全自動を主経路にし、**v2 が出た回だけ「サイトで手動」に切り替える**構成が現実的
  (reserve.js は reCAPTCHA の JS を触らない。v2 が出たら送信せず rejected 相当で人間に委ねる)。
  noVNC は「v2 の回もスマホで完結させたい」場合のみ追加する重い選択肢

## 人間らしい操作版の検証(2026-09-03 夜、Actions dry-run 1件ずつ8回)

reserve.js をマウス移動・1文字ずつ入力・操作間の小休止を入れる形にした(トークン・ブラウザの正体は偽装しない)。

| 回 | reCAPTCHA | 所要 |
|---|---|---|
| 1〜7 | **v3のみ ×7** | 59〜89秒(機械操作版は約46秒) |
| 8 | 判定不能(ログイン直後にサイトのホーム画面 Ajax が失敗し、読み直しでログイン状態に戻れず中断。reCAPTCHA とは無関係) | - |

- 機械操作版の後半9回と合わせ、**直近16回連続で v3(チェック不要)**。v2 が出たのは当日早い時間の2回のみ
- 人間らしい操作の代償は約20〜40秒の遅延。v2 抑止効果は「少なくとも悪化していない」以上は現時点で言えない(v2 が元々まれ)
- 8回目の失敗を受け、ホーム読み直しを `index.jsp` の直接 GET からサイトの「ホーム」操作(`gRsvWOpeHomeAction` へ form1 を POST)に変更
- 「データ通信を正しく行うことができませんでした」の alert はログイン直後のホーム画面で頻出(お気に入り等の Ajax 失敗)。
  公園の選択肢が出ないケースの前兆になっている


## 決定的な発見: v2 チェックボックスは「予約」を押した後に出る(2026-09-03 夜、Actions 本番実行 ×2)

人間らしい操作版で Actions から実予約を2回試みた(いずれも修正後のコード)。結果は **2回とも不成立**。

- 流れ: 予約内容確認画面(この時点では v3 バッジのみ、チェックボックス無し)→「予約」クリック → 確認ダイアログ OK
  → サイトの JS が v3 トークンを取って送信 → **サーバーが同じ予約内容確認画面(prwea1000)を再描画し、
  「チェックを入れてから『予約』ボタンをクリックしてください」と v2 チェックボックスを表示**
- つまり **v2 は v3 のスコアが低いと判定された「送信後」に初めて出る**。送信前の画面を見る dry-run では判定できない。
  → 先に記録した「dry-run 19回中 v3 17回」は **実予約の成功率を意味しない**(誤った指標だった)
- 実予約の成績: **Actions(DC IP)0/3**(STEP 8 + 本日2回。人間らしい操作でも同じ)、**Mac(自宅回線)1/1**
- 結論: **DC IP からの完全自動予約は(人間らしい操作にしても)成立しない**。成立するのは住宅 IP からの実行のみ。
  DC IP を使うなら、送信後に出る v2 を人間が解く半自動(noVNC 等)が必要
- 補足: ログイン直後の「データ通信を正しく行うことができませんでした」でログイン画面に戻るケースは
  一時エラーとして再試行するよう修正(以前は auth_error と誤分類し、LINE に「ログインできませんでした」を誤送信)
- 補足: Actions の artifact が毎回空だったのは、`.debug/` が隠しディレクトリで upload-artifact に除外されていたため。`debug-out/` に変更

## 高速経路(HTTP → ブラウザ引き渡し)の実測(2026-09-03 夜、ローカル dry-run)

`booking/src/site-http.js` で HTTP のみでログイン〜枠選択(6リクエスト)を行い、Cookie とフォーム値をブラウザに移植して
予約内容確認画面への POST だけをブラウザで行う方式。

| 区間 | 所要 |
|---|---|
| HTTP: index → ログイン画面 → ログイン → 検索 → 週JSON → セル選択 | 約20秒(週JSON だけで 6.6秒。サイトの応答が支配的) |
| ブラウザ: 起動 → Cookie 移植 → 確認画面 POST → 人数入力 | 約15秒(うち POST 完了まで 12.8秒) |

- 全ブラウザ方式(39〜60秒)より短縮。以前ローカルで観測した v3 のみの状態は変わらず(DC IP では送信後に v2 が出る前提)
- 予約内容確認画面への POST は、about:blank に置いたフォーム(accept-charset=Shift_JIS、週表示画面の hidden 一式 + applyFlg=1)を submit するだけで通る
  (JSESSIONID は SameSite=None なのでクロスサイト POST でも送られる)
- 利用者番号の形式が不正だとログイン POST は 302 を返す(通常は 200)。ブラウザ側は「利用者番号は半角数字で入力して下さい。」の alert で遷移しない
- 今後の短縮余地: ブラウザ起動(約5秒)を HTTP 区間と並行させる

## Cloud Run 上の高速経路(ブラウザ内 fetch 版)の実測(2026-09-03 夜)

- Node 側 HTTP → ブラウザへ Cookie 移植の方式は、Cloud Run 上では予約内容確認画面への POST が
  「システム異常が発生しました」(週表示画面に戻され selectSize=0)で失敗した(ローカルでは成功)。
  Node の fetch とブラウザの通信が別の出口(IP)になり、サイトの LB がセッションを別ノードに振ったとみられる
- 対策: `src/site-inpage.js`。トップページを開いたブラウザの中で fetch によりログイン〜枠選択を行い、DOM に作ったフォームで
...
## 「予約」を押して確認画面(prwea1000)のままでも、予約は成立していることがある(2026-09-13 21:32、自宅 Pi)

- 流れ: 高速経路で予約内容確認画面 → 「予約」→ 確認ダイアログ OK → **約 8 秒後に遷移完了したが、画面は同じ確認画面(prwea1000)**。
  エラー文・alert・reCAPTCHA のチェックボックスいずれも無し(HTML に g-recaptcha が 0 件。人数 2 は入力済みのまま、上部に loading 画像)
- 当時の回線は不安定(直後 21:33:43 に Cloudflare Tunnel が切断、〜21:37)。サイトの応答も遅かった(枠選択まで 19 秒)
- 従来コードはこれを `error: 完了画面に到達しませんでした` と判定したが、**利用者が「よやく」で確認すると 9/21 19:00 亀戸中央の予約が入っていた**(都からの完了メールも届いた)
- 推定: 予約の POST はサーバーに届いて成立し、応答(完了画面)が回線の乱れで届かないか、ブラウザ側が確認画面の再描画を先に見た。
  どちらにせよ **画面遷移だけで成否を決めてはいけない**
